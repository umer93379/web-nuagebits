      ...
      <div class="mx-auto max-w-screen-xl text-center">
          {footer?.photo?.image && (
            <GatsbyLink to="/" className="flex justify-center items-center text-2xl font-semibold text-gray-900 dark:text-white">
              ...
              />
              {footer?.title && ( <p class="my-6 text-gray-500 dark:text-gray-400">{footer?.title}</p>)}
            </GatsbyLink>
          )}
          {footer?.content && ( <p class="my-6 text-gray-500 dark:text-gray-400">{footer?.content}</p>)}
          ...
          {footer.copyright && (<span class="text-sm text-gray-500 sm:text-center dark:text-gray-400">&copy; {footer.copyright} <a href="/" class="hover:underline">Edge Base</a>. All Rights Reserved.</span>)}
      </div>
    </footer>
  );
}
